...
class TestVoterQueue:
    """Test Voter Queue implementation."""
    
    def test_queue_creation(self):
        queue = VoterQueue(capacity=10)
    ...
    def test_enqueue_dequeue(self):
        ...
        # Enqueue voters
        voter1 = {"voter_id": "V001", "name": "Alice"}
        voter2 = {"voter_id": "V002", "name": "Bob"}
        
        assert queue.enqueue(voter1) == True
        assert queue.enqueue(voter2) == True
        assert queue.get_size() == 2
        
        # Dequeue voters (FIFO)
        first = queue.dequeue()
        assert first["voter_id"] == "V001"
        assert queue.get_size() == 1
    
    def test_queue_stats(self):
        queue = VoterQueue(capacity=10)
        queue.enqueue({"voter_id": "V001"})
...
class TestHashTable:
    ...
    def test_insert_and_search(self):
        ...
        # Insert items
        ht.insert("key1", {"data": "value1"})
        ht.insert("key2", {"data": "value2"})
        
        # Search items
        result1 = ht.search("key1")
        assert result1["data"] == "value1"
        
        result2 = ht.search("key2")
        assert result2["data"] == "value2"
        
        # Search non-existent
        result3 = ht.search("key3")
        assert result3 is None
    
    def test_hash_table_stats(self):
        ht = VoteHashTable(capacity=10)
        ht.insert("key1", {"data": "value1"})
