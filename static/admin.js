    ...
    const proofHtml = `
        <div class="bg-black/20 rounded-lg p-6 border border-blue-400/30 mt-4">
            <h4 class="text-blue-300 font-semibold mb-4 flex items-center">
                <i class="fas fa-certificate mr-2"></i>Ballot Verification Complete
            </h4>
            <div class="space-y-4 text-sm">
                ...
                <div>
                    ...
                    <div class="bg-gray-900 rounded px-3 py-2 font-mono text-green-300 text-xs break-all mt-1">${proofData.seq}</div>
                </div>
                <div>
                    <span class="text-white/70">Verification Status:</span>
...
async function loadDataStructureStats() {
    try {
        ...
        if (response.ok && data.data_structures) {
            displayDataStructureStats(data.data_structures);
        }
    } catch (error) {
        console.error('Failed to load data structure stats:', error);
    }
}

function displayDataStructureStats(stats) {
    const container = document.getElementById('data-structures-stats');
        ...
        html += `
            <div class="bg-blue-900/20 rounded-lg p-3 border border-blue-400/30">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-blue-300 font-semibold"><i class="fas fa-stream mr-2"></i>Queue (FIFO)</span>
                    <span class="text-blue-400 text-xs">O(1) operations</span>
                </div>
                <div class="grid grid-cols-2 gap-2 text-xs">
                    <div><span class="text-white/70">Size:</span> <span class="text-white">${stats.queue.current_size}/${stats.queue.capacity}</span></div>
                    <div><span class="text-white/70">Utilization:</span> <span class="text-white">${stats.queue.utilization_percent.toFixed(1)}%</span></div>
                </div>
            </div>
        `;
    }
    ...
    // Array Stats
    if (stats.candidate_array) {
        html += `
            <div class="bg-green-900/20 rounded-lg p-3 border border-green-400/30">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-green-300 font-semibold"><i class="fas fa-list mr-2"></i>Candidate Array</span>
                    <span class="text-green-400 text-xs">O(n) search</span>
                </div>
                <div class="grid grid-cols-2 gap-2 text-xs">
                    <div><span class="text-white/70">Candidates:</span> <span class="text-white">${stats.candidate_array.current_size}</span></div>
                    <div><span class="text-white/70">Total Votes:</span> <span class="text-white">${stats.candidate_array.total_votes}</span></div>
                </div>
            </div>
        `;
    }
    ...
    if (stats.hash_table) {
        html += `
            <div class="bg-purple-900/20 rounded-lg p-3 border border-purple-400/30">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-purple-300 font-semibold"><i class="fas fa-hashtag mr-2"></i>Hash Table</span>
                    <span class="text-purple-400 text-xs">O(1) avg lookup</span>
                </div>
                <div class="grid grid-cols-2 gap-2 text-xs">
                    <div><span class="text-white/70">Load Factor:</span> <span class="text-white">${stats.hash_table.load_factor.toFixed(3)}</span></div>
                    <div><span class="text-white/70">Max Chain:</span> <span class="text-white">${stats.hash_table.max_chain_length}</span></div>
                </div>
            </div>
        `;
    }
    ...
    // Stack Stats
    if (stats.audit_stack) {
        html += `
            <div class="bg-orange-900/20 rounded-lg p-3 border border-orange-400/30">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-orange-300 font-semibold"><i class="fas fa-layer-group mr-2"></i>Audit Stack (LIFO)</span>
                    <span class="text-orange-400 text-xs">O(1) push/pop</span>
                </div>
                <div class="grid grid-cols-2 gap-2 text-xs">
                    <div><span class="text-white/70">Events:</span> <span class="text-white">${stats.audit_stack.total_events}</span></div>
                    <div><span class="text-white/70">Empty:</span> <span class="text-white">${stats.audit_stack.is_empty ? 'Yes' : 'No'}</span></div>
                </div>
            </div>
        `;
    }
    
    container.innerHTML = html || '<div class="text-white/50 text-sm">No stats available</div>';
}
