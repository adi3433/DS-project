                    ...
                    <div class="bg-black/20 rounded-lg p-4 border border-green-400/30">
                        ...
                        <div class="space-y-2 text-sm">
                            ...
                            <div>
                                ...
                                <div class="bg-gray-900 rounded px-3 py-2 font-mono text-purple-300 text-xs break-all mt-1">${result.timestamp}</div>
                            </div>
                        </div>
                    </div>
                </div>
...
async function loadResults() {
    try {
        ...
        if (response.ok) {
            displayResults(data);
            // Data structure stats loaded separately
        } else {
...
function displayStats(data) {
    ...
    if (data.data_structures) {
        html += `
            <div class="bg-blue-50 p-4 rounded">
                <h4 class="font-semibold text-blue-800">Queue Size</h4>
                <p class="text-2xl font-bold text-blue-600">${data.data_structures.queue?.current_size || 0}</p>
                <p class="text-sm text-blue-600">Voters in queue</p>
            </div>
            <div class="bg-green-50 p-4 rounded">
                <h4 class="font-semibold text-green-800">Hash Table</h4>
                <p class="text-2xl font-bold text-green-600">${data.data_structures.hash_table?.load_factor?.toFixed(3) || 0}</p>
                <p class="text-sm text-green-600">Load factor</p>
            </div>
        `;
    }
...
async function undoLast() {
    ...
    try {
        ...
        if (response.ok) {
            showMessage(resultDiv, 
                `Successfully undone: ${result.undone_action}. ${result.message}`, 'success');
        } else {
...
function displayProof(proofData) {
    ...
    let html = `
        <div class="bg-green-50 border border-green-200 rounded-lg p-4">
            <h4 class="font-semibold text-green-800 mb-2">Ballot Verification</h4>
            <div class="space-y-2 text-sm">
                <div><strong>Ballot Hash:</strong> <code class="bg-gray-100 px-2 py-1 rounded">${proofData.ballot_hash}</code></div>
                <div><strong>Sequence Number:</strong> ${proofData.seq}</div>
